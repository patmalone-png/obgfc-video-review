...
export default function App(){
  ...
  const [notes,setNotes]=useState(()=>{ try{return JSON.parse(localStorage.getItem("obgfcV8Notes")||"[]");}catch{return [];} });
  const [season,setSeason]=useState(()=>{ try{return JSON.parse(localStorage.getItem("obgfcV8Season")||"[]");}catch{return [];} });

  const [aiEndpoint,setAiEndpoint]=useState(()=>localStorage.getItem("obgfcV8AiEndpoint")||"/api/coach");
  const [teamSecret,setTeamSecret]=useState(()=>localStorage.getItem("obgfcV8TeamSecret")||"");
  ...
  useEffect(()=>localStorage.setItem("obgfcV8Notes", JSON.stringify(notes)),[notes]);
  useEffect(()=>localStorage.setItem("obgfcV8Season", JSON.stringify(season)),[season]);
  useEffect(()=>localStorage.setItem("obgfcV8AiEndpoint", aiEndpoint),[aiEndpoint]);
  useEffect(()=>localStorage.setItem("obgfcV8TeamSecret", teamSecret),[teamSecret]);
  ...
  function exportAudio(){ if(!audioUrl) return; const a=document.createElement("a"); a.href=audioUrl; a.download="obgfc-v8-audio.webm"; document.body.appendChild(a); a.click(); a.remove(); }
  ...
  function exportMarkdown(){ download("obgfc-v8-match-report.md", summaryText(), "text/markdown"); }
  function exportCsv(){ const h=["Source","Match","Opponent","Venue","Quarter","Clock","Scope","Player","Theme","Tone","Tag","Raw","Interpretation","Created At"]; const rows=notes.map(n=>[n.source,n.matchTitle,n.opponent,n.venue,n.quarter,n.clockLabel,n.scope,n.player,n.theme,n.tone,n.tag,n.raw,n.interpretation,n.createdAt]); download("obgfc-v8-notes.csv", [h,...rows].map(r=>r.map(csvEscape).join(",")).join("\n"), "text/csv"); }
  function exportJson(){ download("obgfc-v8-season-data.json", JSON.stringify({matchTitle,opponent,venue,notes,season},null,2), "application/json"); }
  function exportWord(){ const stop = stopOutput?`<h2>STOP5 Review</h2><p>${safeHtml(stopOutput).replaceAll("\n","<br/>")}</p>`:""; const extra = aiOutput?`<h2>AI Coach Summary</h2><p>${safeHtml(aiOutput).replaceAll("\n","<br/>")}</p>`:""; const body=safeHtml(summaryText()).replaceAll("\n","<br/>"); const html=`<!doctype html><html><head><meta charset="utf-8"><title>${safeHtml(matchTitle)}</title><style>body{font-family:Arial,sans-serif;color:#111;line-height:1.4}h1{color:#0f172a}h2{color:#075985;border-bottom:1px solid #ccc;padding-bottom:4px}</style></head><body>${body}<br/>${stop}${extra}</body></html>`; download("obgfc-v8-match-report.doc", html, "application/msword"); }
  ...
  async function callProxy(payload, mode){
    const headers={ "Content-Type":"application/json" };
    if(teamSecret) headers["x-coach-secret"]=teamSecret;
    const res=await fetch(aiEndpoint || "/api/coach", { method:"POST", headers, body: JSON.stringify({ payload, mode }) });
  ...
  function builtinAi(){
    ...
    return ["COACHING SUMMARY (built-in fallback)","",`Strongest themes: ${top}.`,`Priority improvements: ${train}.`,"","Connect the secure endpoint for a full AI report."].join("\n");
  }
  async function runAi(){
    setAiBusy(true); setAiOutput(""); setAiInfo("");
    try{ const data=await callProxy(buildAiPayload(),"default"); setAiOutput(data.summary||"No response."); setAiInfo(`Engine: ${data.provider||"?"}`); }
    catch(err){ setAiOutput(`AI request failed. ${err.message}\n\nFallback:\n\n${builtinAi()}`); setAiInfo("Fell back to built-in"); }
    finally{ setAiBusy(false); }
  }

  // ---------- STOP5 (replay verbatims) ----------
  function buildStop5Payload(){
    // Use replay verbatims if present, else all notes. Cap to the most recent `stopLimit`
    // so a large session (100+ notes) does not blow the model's output budget.
    const source = replayNotes.length ? replayNotes : notes;
    const list = source.slice(0, stopLimit); // notes are newest-first
    const header = `MATCH: ${matchTitle}\nOPPONENT: ${opponent||"-"}\nVENUE: ${venue||"-"}\n\nCOACH VERBATIM COMMENTARY (from video replay, most recent first, ${list.length} of ${source.length} shown):`;
    const body = [...list].map(n=>`[${n.clockLabel}${n.quarter?` ${n.quarter}`:""}${n.player?` #${n.player}`:""}] ${n.raw}`).join("\n");
    return `${header}\n${body || "No verbatim commentary captured yet."}`;
  }
  async function runStop5(){
    setStopBusy(true); setStopOutput(""); setStopInfo("");
    const source = replayNotes.length ? replayNotes : notes;
    if(!source.length){ setStopOutput("No commentary captured yet. Start commentary in the Replay tab, then run the STOP5 summary."); setStopBusy(false); return; }
    try{
      const used = Math.min(stopLimit, source.length);
      const data=await callProxy(buildStop5Payload(),"stop5");
      setStopOutput(data.summary||"No response.");
      setStopInfo(`Engine: ${data.provider||"?"} | analysed ${used} of ${source.length} verbatims`);
    }
    catch(err){ setStopOutput(`STOP5 request failed. ${err.message}`); setStopInfo("Error"); }
    finally{ setStopBusy(false); }
  }
  function copyText(t){ if(t) navigator.clipboard?.writeText(t); }
  function exportStop5Word(){ const html=`<!doctype html><html><head><meta charset="utf-8"><title>STOP5 Review</title><style>body{font-family:Arial,sans-serif;color:#111;line-height:1.5}h1{color:#0f172a}</style></head><body><h1>${safeHtml(matchTitle)} - STOP5 Review</h1><p>${safeHtml(stopOutput).replaceAll("\n","<br/>")}</p></body></html>`; download("obgfc-v8-stop5-review.doc", html, "application/msword"); }
  function exportAiWord(){ const html=`<!doctype html><html><head><meta charset="utf-8"><title>AI Coach Summary</title><style>body{font-family:Arial,sans-serif;color:#111;line-height:1.5}h1{color:#0f172a}</style></head><body><h1>${safeHtml(matchTitle)} - AI Coach Summary</h1><p>${safeHtml(aiOutput).replaceAll("\n","<br/>")}</p></body></html>`; download("obgfc-v8-ai-coach-summary.doc", html, "application/msword"); }
    ...
    <div className="app">
      <div className="wrap">
        <header className="hero">
          ...
          <h1>V8 Coaching Intelligence</h1>
          <p>Replay review with a Gemini STOP5 debrief, live match-day capture, a season dashboard, and a secure AI coach.</p>
          <div className="tabs">
            <button className={tab==="replay"?"tab active":"tab"} onClick={()=>{setTab("replay"); setStatus("Replay Review: comment while you watch, then run a STOP5 summary.");}}>Replay Review</button>
            ...
            <button className={tab==="season"?"tab active":"tab"} onClick={()=>{setTab("season"); setStatus("Season Dashboard: track themes and get an AI coaching summary.");}}>Season Dashboard</button>
          </div>
        ...
        <main className="grid two top">
          <section className="card no-print">
            ...
            <div className="sub top">
              <h2>{tab==="replay"?"Commentary":"Live mic"}</h2>
              <div className="actions">
                {!listening?<button className="orange" onClick={startSpeech}>{tab==="replay"?"Start commentary":"Start transcription"}</button>:<button className="red" onClick={stopSpeech}>Stop</button>}
              </div>
              <div className="note live top-sm">{live||"Live transcript appears here. Final phrases become timestamped calls."}</div>
            </div>

            {tab==="replay" && (
              <div className="sub top stop5">
                <div className="actions space">
                  <h2>Gemini STOP5 Review</h2>
                  <span className="pill">{replayNotes.length} replay verbatim{replayNotes.length===1?"":"s"}</span>
                </div>
                <p className="muted">Sends your replay commentary to Gemini and returns a structured debrief: Situation, Things that are working, Opportunities to improve, Points of action.</p>
                <div className="actions">
                  <button className="green" onClick={runStop5} disabled={stopBusy}>{stopBusy?"Interpreting verbatims…":"Generate STOP5 summary"}</button>
                  <label className="inline"><span>Analyse latest</span>
                    <select value={stopLimit} onChange={e=>setStopLimit(Number(e.target.value))}>
                      <option value={30}>30 verbatims</option>
                      <option value={60}>60 verbatims</option>
                      <option value={100}>100 verbatims</option>
                      <option value={9999}>All</option>
                    </select>
                  </label>
                  {stopOutput && <button onClick={()=>copyText(stopOutput)}>Copy</button>}
                  {stopOutput && <button className="blue" onClick={exportStop5Word}>Export Word</button>}
                </div>
                {stopInfo && <p className="muted top-sm">{stopInfo}</p>}
                <div className="note ai top-sm">{stopOutput || "Your STOP5 debrief will appear here."}</div>
              </div>
            )}
        ...
        <main className="grid top">
          <section className="card">
            ...
            {seasonData.gameCount===0 ? (
              <p className="muted">No saved games yet. Capture notes then press <b>Save snapshot</b>. Each saved game feeds these season trends.</p>
            ) : (
              <>
                <div className="grid four top">
          ...
          <section className="card top">
            ...
            <p className="muted">Runs through your Vercel serverless function. The API key stays on the server. Free with a Gemini key.</p>
            <div className="grid four top">
              ...
              <label><span>Endpoint</span><input value={aiEndpoint} onChange={e=>setAiEndpoint(e.target.value)} placeholder="/api/coach" /></label>
              <label className="wide"><span>Team secret (optional)</span><input type="password" value={teamSecret} onChange={e=>setTeamSecret(e.target.value)} placeholder="Only if you set COACH_SHARED_SECRET" /></label>
            </div>
            <div className="actions top-sm">
              ...
              {aiOutput && <button onClick={()=>copyText(aiOutput)}>Copy</button>}
