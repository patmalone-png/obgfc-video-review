...
export default function App(){
  ...
  function builtinAi(){
    ...
    const lines = ["COACHING SUMMARY (built-in fallback)","",`Strongest themes: ${top}.`,`Priority improvements: ${train}.`,"","Connect the secure endpoint for a full AI report."];
    return lines.join("\n");
  }
  ...
  function buildStop5Payload(){
    const list = replayNotes.length ? replayNotes : notes;
    const header = `MATCH: ${matchTitle}\nOPPONENT: ${opponent||"-"}\nVENUE: ${venue||"-"}\n\nCOACH VERBATIM COMMENTARY (from video replay, most recent first):`;
  ...
  async function runStop5(){
    ...
    if(!replayNotes.length && !notes.length){ setStopOutput("No commentary captured yet. Start commentary in the Replay tab, then run the STOP5 summary."); setStopBusy(false); return; }
    try{ const data=await callProxy(buildStop5Payload(),"stop5"); setStopOutput(data.summary||"No response."); setStopInfo(`Engine: ${data.provider||"?"} | ${replayNotes.length||notes.length} verbatims`); }
