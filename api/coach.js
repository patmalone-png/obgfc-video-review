...
// Securely proxies coaching-summary requests to OpenAI or Anthropic.
// The API key is read from environment variables on the server and is
...
// Optional:
//   AI_PROVIDER = "openai" | "anthropic"  (defaults to whichever key exists)
...
export default async function handler(req, res) {
  // Basic CORS (same-origin in production, permissive for convenience)
  ...
  try {
    ...
    const provider =
      (process.env.AI_PROVIDER || (openaiKey ? "openai" : anthropicKey ? "anthropic" : "")).toLowerCase();

    if (!provider) {
      return res.status(500).json({
        error:
          "No AI provider configured. Add OPENAI_API_KEY or ANTHROPIC_API_KEY in Vercel environment variables.",
      });
    }
